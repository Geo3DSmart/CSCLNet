...
class Attention(nn.Module):
    def __init__(self, dim, heads = 8, dim_head = 64, dropout = 0.):    # 都是[B, seq_len, dim]
        super().__init__()
        inner_dim = dim_head *  heads
        self.heads = heads
        self.scale = dim ** -0.5
...
if __name__ == '__main__':
    input = torch.randn(10,8,32)
    from CiT.CiT_Net_T import WindowAttention_ACAM
    ac = WindowAttention_ACAM(dim=32, num_heads=8, window_size=8)
    danet = Attention(dim=32, heads=8, dim_head=64, dropout=0)
    print(danet(input).shape)

    from CiT.CiT_Net_T import WindowAttention_ACAM

    model = WindowAttention_ACAM(dim=32, num_heads=8,window_size=8)
    input = torch.randn(10, 8, 32)
    output = model(input)
    print(output.shape)
